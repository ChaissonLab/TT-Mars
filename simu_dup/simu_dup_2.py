...
if_hg38_input = "True"
centromere_file = "./centromere_hg38.txt"
#assembly bam files
...
altered_vcf_file = output_dir + "/simu_dup_1000.vcf"
...
#Output sv positions
get_conf_int.get_sv_positions(output_dir, altered_vcf_file)
...
f = pysam.VariantFile(altered_vcf_file,'r')
sv_list = []
...
#     #test: for current lra only
#     if test_sv.length < 1000:
#         continue
...
aligner_hap1 = mappy.Aligner(fn_idx_in=altered_query_file1)
aligner_hap2 = mappy.Aligner(fn_idx_in=query_file2)

##################################################################################
##################################################################################

altered_query_fasta_file1 = pysam.FastaFile(altered_query_file1)
ref_name_list_1, ref_pos_list_1, contig_idx_len_1 = build_map_asm_to_ref(altered_query_fasta_file1, altered_liftover_file1_0)

#get asm to ref mapping
ref_name_list_2, ref_pos_list_2, contig_idx_len_2 = build_map_asm_to_ref(query_fasta_file2, liftover_file2_0)
...
for agt_list in alignment_list:
    ...
    #         continue
        ...
        if int_start == -1 or int_end == -1:
            ...
            sv_list[sv_idx].analyzed_hap1 = False
            sv_list[sv_idx].analyzed_hap2 = False
            break
        ...
        if int_start != -1 and int_end != -1:
            ...
            hap = agt.hap
            sv_list[sv_idx].analyzed_hap1 = True
            sv_list[sv_idx].analyzed_hap2 = True
            ...
            if agt.cal_ins_rela_len() > ins_rela_len_lb and agt.cal_ins_rela_len() < ins_rela_len_ub:
                ...
                sv_list[sv_idx].valid_non_ins = True        
